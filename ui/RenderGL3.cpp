...
//
bool RenderGLInit(const char* fontpath,int*out)
{
	for (int i = 0; i < CIRCLE_VERTS; ++i)
	{
...
/*for(int i=0;i<512*512;++i){
	...
	glBindTexture(GL_TEXTURE_2D, g_whitetex);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_ALPHA, 1, 1, 0, GL_ALPHA, GL_UNSIGNED_BYTE, &white_alpha);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
  ...
  glBindBuffer(GL_ARRAY_BUFFER, 0);
	...
	#ifndef __gl3_h_
	if(glBindFragDataLocation)glBindFragDataLocation(g_program, 0, "Color");
	#endif
